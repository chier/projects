...
var ICON_SIZE = 70;  //截取框的大小，正方形
var LEFT_EDGE = (CANVAS_WIDTH - 70) / 2; //107
var TOP_EDGE = (CANVAS_HEIGHT - 70) / 2; //121
...
$(window).load(function() {
    ...
    var originTop = Math.round((CANVAS_HEIGHT - currentHeight)/ 2+98);
    //计算截取框中的图片的位置
    var dragleft = Math.round(originLeft - LEFT_EDGE);
    var dragtop = Math.round(originTop - TOP_EDGE-120);
    ...
    $imagedrag.draggable(
    {
        ...
        drag: function(e, ui) {
            ...
            var top1=parseInt(self.position.top) + TOP_EDGE;
            var left = drop_img.css("left").replace(/px/, ""); //取出截取框到左边的距离
            drop_img.css({ left: (parseInt(self.position.left) + LEFT_EDGE) + "px", top: (top1+118) + "px" }); //同时移动内部的图片
            $("#txt_left").val(Math.round(0 - parseInt(self.position.left)));
            $("#txt_top").val(Math.round(0 - parseInt(self.position.top)));
        }
    }
    );
    //设置图片可拖拽，并且拖拽一张图片时，同时移动另外一张图片
    $iconElement.draggable(
    {
        ...
        drag: function(e, ui) {
            ...
            var left = drop_img.css("left").replace(/px/, ""); //取出截取框到左边的距离
            drop_img.css({ left: (parseInt(self.position.left) - LEFT_EDGE) + "px", top: (top1-118) + "px" }); //同时移动内部的图片
            $("#txt_left").val(Math.round(0 - (parseInt(self.position.left) - LEFT_EDGE)));
            $("#txt_top").val(Math.round(0 -(parseInt(self.position.top) - TOP_EDGE - 118)));
        }
    ...
    $(".child").draggable(
  {
      ...
      var left = parseInt($(this).css("left"));
          ...
          scaleFactor = Math.pow(factor, (left / 100 - 1));
	        ...
	        //缩小判断是否最小化
	        if (scaleFactor < minFactor || 0.04930066485916347 == scaleFactor) {
	        	var h = base +"/images/imge/_h.gif";
	        	$("#moresmall").attr("src",h); 
	            scaleFactor = minFactor;
	        }else{
	        	var c = base +"/images/imge/_c.gif";
	        	$("#moresmall").attr("src",c); 
	        }
	        
	        // 放大判断是否最大化
	        if (scaleFactor >= factor) {
	        	var h1 = base + "/images/imge/h.gif";
	        	$("#morebig").attr("src",h1); 
	            scaleFactor = factor;
	        }else{
	        	var c1 = base + "/images/imge/c.gif";
	        	$("#morebig").attr("src",c1); 
	        }
          ...
          dragtop = originTop - TOP_EDGE-120;
    ...
    var num = 0 ;
    while((parseInt($('#txt_width').val())<parseInt($('#txt_DropWidth').val())||parseInt($('#txt_height').val())<parseInt($('#txt_DropHeight').val()))&&num<4){
    	SilderSetValue(20);
    	num++;
    }
});