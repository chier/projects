...
WebFXTree.prototype.toString = function() {
	...
	strArray.push("\" ondblclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\">");
	strArray.push("<input type=\"checkbox\" class=\"tree-check-box\"");
	strArray.push((this._checked ? " checked=\"checked\"" : ""));
	strArray.push(" id=\"");
	strArray.push(this.id);
	strArray.push("-input\"");
	strArray.push(" onclick=\"checkRootNode(webFXTreeHandler.all[this.parentNode.id],this.checked)\" />");
	
	strArray.push("<img id=\"");
	strArray.push(this.id);
	strArray.push("-icon\" class=\"webfx-tree-icon\" src=\"");
	strArray.push(((webFXTreeHandler.behavior == 'classic' && this.open)?this.openIcon:this.icon));
	strArray.push("\" >");	
	strArray.push("<a id=\"");
	strArray.push(this.id);
	strArray.push("-anchor\" ");
	...
	strArray.push(";\">");	
	for (var i = 0,childNodesLength = this.childNodes.length; i < childNodesLength; i++) {
	...
	var str = "<div id=\"" + this.id + "\" ondblclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\">" ;
		...
		//if (this.action != "folder" && this.sIsRootCheckBox == "true"){	
			str += "<input type=\"checkbox\"" +
				...
				(this._checked ? " checked=\"checked\"" : "") + " id=\""+this.id+"-input\""+
				" onclick=\"checkRootNode(webFXTreeHandler.all[this.parentNode.id],this.checked)\"" +
				" />" ;
...
function checkRootNode(node,bChecked){
	node._checked = bChecked;	
	doCheck(node, bChecked);
	if (typeof node.onchange == "function"){
		node.onchange(node.text, node.value, bChecked);
	}else if(node.onchange != null && node.onchange != ""){
		var str = node.onchange + "('" + node.text + "','" + node.value + "'," + bChecked + ");";
		eval(str);
	}
}
