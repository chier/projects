...
function WebFXLoadCheckBoxTreeItem(sText, sValue, isItem,sXmlSrc, eParent, sIcon, sOpenIcon, bChecked, disabled) {
	// call super	
	...
	this.base(sText, sValue, isItem,eParent, sIcon, sOpenIcon, bChecked, disabled);
...
function checkbox_startLoadXmlTree(sSrc, jsNode) {
	...
	xmlHttp.open("GET", sSrc, false);	//s not async
	...
	if (window.XMLHttpRequest) {
		...
		//		checkbox_hide();
				...
				var childrennodes = checkbox_xmlFileLoaded(xmlHttp.responseXML, jsNode);
				if(childrennodes && childrennodes.length> 0){
					for(var i = 0 ; i < childrennodes.length ; i ++){
						checkbox_startLoadXmlTree(childrennodes[i].src,childrennodes[i]);
					}
				}
			}
		};
	...
	if (window.ActiveXObject) {
		...
		//		checkbox_hide();
				...
				if(childrennodes && childrennodes.length> 0){
					for(var i = 0 ; i < childrennodes.length ; i ++){
						if (childrennodes[i] != null && childrennodes[i].src != null) {
						    checkbox_startLoadXmlTree(childrennodes[i].src,childrennodes[i]);
					    }
					}
				}
...
function checkbox_createNode(oNode, parentDisabled){
	...
	var isItem = oNode.getAttribute("isItem");
	...
	var jsNode;
	if (src != null && src != "")
		jsNode = new WebFXLoadCheckBoxTreeItem(text, value, isItem,src, parent, icon, openIcon, checked, disabled);
	else
		jsNode = new WebFXCheckBoxTreeItem(text, value,isItem, parent, icon, openIcon, checked, disabled);
   
    // 将选中的值加入结果集中.
	if(checked){
		checkedValues.put(jsNode.id,  new CheckedObject(jsNode.id, text, value));  // 这里的id是系统自动生成的id，不是oid（业务对象id）。
		checkedNodes.put(jsNode.id,  jsNode);	
	}
