...
WebFXTreeAbstractNode.prototype.add = function(node, bNoIdent) {
	...
	if (root.rendered) {
		if (this.childNodes.length >= 2) {
			document
					.getElementById(this.childNodes[this.childNodes.length - 2].id
							+ '-plus').src = ((this.childNodes[this.childNodes.length
					- 2].folder)
		...
		webFXTreeHandler.insertHTMLBeforeEnd(document.getElementById(this.id
						+ '-cont'), node.toString());
...
WebFXTreeAbstractNode.prototype.doExpand = function() {
	...
	if (webFXTreeConfig.usePersistence) {
		webFXTreeHandler.cookies.setCookie(this.id.substr(18, this.id.length
								- 18), '1');
	}
}
...
WebFXTreeAbstractNode.prototype.indent = function(lvl, del, last, level,
	 ...
	 * to -2
	 * 
	 * if (last){ document.getElementById(this.id + '-plus').src
	 * =webFXTreeConfig.openFolderIcon; } alert(lvl + ":" + del + ":" +last +
	 * ":" +level + ":" +last );
	 */
	if (lvl == null) {
		lvl = -2;
	...
	if (del) {
		if ((level >= this._level)
				&& (document.getElementById(this.id + '-plus'))) {
			if (this.folder) {
			...
			// if (last) {
			// document.getElementById(this.id + '-icon').src
...
function WebFXTree(sText, sAction, sBehavior, sIcon, sOpenIcon) {
	...
	/* Defaults to open */
	if (webFXTreeConfig.usePersistence) {
		this.open = (webFXTreeHandler.cookies.getCookie(this.id.substr(18,
				this.id.length - 18)) == '0') ? false : true;
	} else {
		this.open = true;
	}
	this.folder = true;
...
WebFXTree.prototype.toString = function() {
	...
	strarray.push("\" class=\"webfx-tree-item\" onkeydown=\"return webFXTreeHandler.keydown(this, event)\"  name=\"");
	strarray.push(pushid(this));
	strarray.push("\" onmousedown=right_pop3(this)>");
	...
	strarray.push(((webFXTreeHandler.behavior == 'classic' && this.open)
					? this.openIcon
					: this.icon));
	strarray.push("\" onclick=\"webFXTreeHandler.select(this);\">");				
	strarray.push("<a onclick=\"setColor(this);\" href=\"");			
	strarray.push(this.action);
	strarray.push("\" id=\"");
	strarray.push(this.id);
	strarray.push("-anchor\" onblur=\"webFXTreeHandler.blur(this);\"");
	strarray.push((this.target ? " target=\"" + this.target + "\"" : ""));
	strarray.push(">");
	...
	var str = "<div id=\""
			+ this.id
			+ "\" class=\"webfx-tree-item\" onkeydown=\"return webFXTreeHandler.keydown(this, event)\"  name=\""
			+ pushid(this)
			+ "\" onmousedown=right_pop3(this)>"
			+ "<img id=\""
			+ this.id
			...
			+ "<a onclick=\"setColor(this);\" href=\""
			+ this.action
			+ "\" id=\""
			+ this.id
			+ "-anchor\" onblur=\"webFXTreeHandler.blur(this);\""
	...
	*/
	var sb = [];
	for (var i = 0,l=this.childNodes.length; i < l; i+=1) {
...
function WebFXTreeItem(sText, sAction, eParent, sIcon, sOpenIcon) {
	...
	if (webFXTreeConfig.usePersistence) {
		this.open = (webFXTreeHandler.cookies.getCookie(this.id.substr(18,
				this.id.length - 18)) == '1') ? true : false;
	} else {
		this.open = false;
	}
	if (sIcon) {
...
WebFXTreeItem.prototype.remove = function() {
	...
	var last = ((nextSibling) && (nextSibling.parentNode) && (nextSibling.parentNode.id == parentNode.id))
			? false
			: true;
	...
	for (var i = 0; i < this.parentNode.childNodes.length; i++) {
		if (this == this.parentNode.childNodes[i]) {
			for (var j = i; j < this.parentNode.childNodes.length; j++) {
				this.parentNode.childNodes[j] = this.parentNode.childNodes[j
						+ 1];
			}
...
WebFXTreeItem.prototype.toString = function(nItem, nItemCount) {
	...
	var isSapnOrA;
	if (label == 'everyone') {
	  isSapnOrA = "<span id=\"" + this.id
				+ "-anchor\" onblur=\"webFXTreeHandler.blur(this);\" style=\"color: #939393;margin-left: 3px\" >"
				+ label + "</span>";
	} else {
		isSapnOrA = "<a onclick=\"setColor(this);\" href=\"" + this.action + "\" id=\"" + this.id
				+ "-anchor\"onblur=\"webFXTreeHandler.blur(this);\""
				+ (this.target ? " target=\"" + this.target + "\"" : "") + ">"
				+ label + "</a>";
	}
	...
	strArray.push("<div id=\"");
	strArray.push(this.id);
	strArray.push("\" onclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\" onkeydown=\"return webFXTreeHandler.keydown(this, event)\" name=\"");
	strArray.push(pushid(this));
	strArray.push("\" onmousedown=right_pop3(this)>");
	strArray.push(indent);
	strArray.push("<img id=\"");
	strArray.push(this.id);
	...
	strArray.push(((webFXTreeHandler.behavior == 'classic' && this.open)
					...
					: this.icon));
	strArray.push("\" onclick=\"webFXTreeHandler.select(this);\">");	
	strArray.push(isSapnOrA);
	strArray.push("</div><div id=\"");
	strArray.push(this.id);
	strArray.push("-cont\" class=\"webfx-tree-container\" style=\"display: ");
	strArray.push(((this.open) ? 'block' : 'none'));
	strArray.push(";\">");
	...
	var str = "<div id=\""
			+ this.id
			+ "\" onclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\" onkeydown=\"return webFXTreeHandler.keydown(this, event)\" name=\""
			+ pushid(this)
			+ "\" onmousedown=right_pop3(this)>"
			+ indent
			+ "<img id=\""
			...
			+ ((webFXTreeHandler.behavior == 'classic' && this.open)
					? this.openIcon
					: this.icon)
			+ "\" onclick=\"webFXTreeHandler.select(this);\">" + isSapnOrA+ "</div>" + "<div id=\"" + this.id
			+ "-cont\" class=\"webfx-tree-container\" style=\"display: "
			+ ((this.open) ? 'block' : 'none') + ";\">";
	*/
	var sb = [];
