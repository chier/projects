...
var webFXTreeConfig = {
	...
	fileIcon        : base + '/js/tree/checkboxtree/images/file.gif',
	...
	defaultCheckBoxValue: 'identifier',
	isopen 			: false,
...
function WebFXTreeAbstractNode(sText, sAction, sCheckBoxValue) {
	this.childNodes  = [];
	this.id     = webFXTreeHandler.getId();
	this.text   = sText || webFXTreeConfig.defaultText;
	this.action = sAction || webFXTreeConfig.defaultAction;
	this.checkBoxValue = sCheckBoxValue || webFXTreeConfig.defaultCheckBoxValue;
	this._last  = false;
...
WebFXTreeAbstractNode.prototype.indent = function(lvl, del, last, level, nodesLeft) {
	...
	if(this.action != "folder" && this.parentNode != undefined){
...
function WebFXTree(sText, sIdentifier, sIsRootCheckBox, sAction, sCheckBoxValue, sBehavior, sIcon, sOpenIcon) {
	this.base = WebFXTreeAbstractNode;
	this.base(sText, sAction, sCheckBoxValue);
	this.sIdentifier = sIdentifier;
	this.sIsRootCheckBox = sIsRootCheckBox;
...
WebFXTree.prototype.toString = function() {
	var str = "<div id=\"" + this.id + "\" ondblclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\">" ;
		// insert check box
		if (this.action != "folder" && this.sIsRootCheckBox == "true"){	
...
function WebFXTreeItem(sText, sAction, sCheckBoxValue, eParent, sIcon, sOpenIcon) {
	this.base = WebFXTreeAbstractNode;
	this.base(sText, sAction, sCheckBoxValue);
	/* Defaults to close */
	if (webFXTreeConfig.usePersistence) {
...
WebFXTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	/** 设置 icon 图片的路径对象 */
	var img_icon = "";
	if(webFXTreeConfig.isopen == true ||(webFXTreeHandler.behavior == 'classic' && this.action == "folder")) {
	...
	var str = "<div id=\"" + this.id + "\" ondblclick=\"webFXTreeHandler.toggle(this);\" class=\"webfx-tree-item\" >";
		...
		if (this.action != "folder"){	
			str += "<input type=\"checkbox\"" +
				" class=\"tree-check-box\"" +
				(this._checked ? " checked=\"checked\"" : "") +
				" id=\"" + this.id  + "-input\" name=\"webfx-tree-object-" + this.checkBoxValue  
		+ "-name\" onclick=\"webFXTreeHandler.all[this.parentNode.id].setChecked(this.checked);handleCheck( this ,'" + this.checkBoxValue + "' ,'"+ this.text + "') \"" +
				" />";
		}
		// end insert checkbox
	    str += "<img id=\"" + this.id + "-icon\" class=\"webfx-tree-icon\" src=\"" + img_icon + "\" onclick=\"webFXTreeHandler.select(this);\">" +
		"<a id=\"" + this.id + "-anchor\" " +
		(this.target ? " target=\"" + this.target + "\"" : "") +
		">" + label + "</a></div>" +
		"<div id=\"" + this.id + "-cont\" class=\"webfx-tree-container\" style=\"display: " + ((this.open)?'block':'none') + ";\">";
...
function setCheckBox(value, checked){
	var cbxs = document.getElementsByName("webfx-tree-object-"+value + "-name");
	if(cbxs && cbxs.length >0){
		for(var i=0; i<cbxs.length; i++){
			var cbx = cbxs[i];
			   ...
			   handleCheck(cbx, value, node.text)
		}
	}
}

// 设置CheckBox的可用状态.
function disableCheckBox(value, checked,disabled){
	var cbxs = document.getElementsByName("webfx-tree-object-"+value + "-name");
	if(cbxs && cbxs.length >0){
		for(var i=0; i<cbxs.length; i++){
			var cbx = cbxs[i];
