...
function WebFXLoadTree(sText, sIdentifier, sIsRootRadio, sXmlSrc, sAction, sBehavior, sIcon, sOpenIcon) {
	// call super
	//alert(sXmlSrc +  " :  " +  sAction);
	this.WebFXTree = WebFXTree;
	this.WebFXTree(sText, sIdentifier, sIsRootRadio, sAction, sBehavior, sIcon, sOpenIcon);
...
function WebFXLoadTreeItem(sText, sXmlSrc, sAction, sRadioValue, isleaf , eParent, sIcon, sOpenIcon,actionDepart) {
	// call super
	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(sText, sAction, sRadioValue,isleaf , eParent, sIcon, sOpenIcon,actionDepart);
...
function _xmlTreeToJsTree(oNode) {
	...
	var radioValue = oNode.getAttribute("identifier");
	var parent = null;
	var icon = oNode.getAttribute("icon");
	var openIcon = oNode.getAttribute("openIcon");
	var src = oNode.getAttribute("src");
	var target = oNode.getAttribute("target");
	var actionDepart = oNode.getAttribute("actionDepart");
	var isleaf = oNode.getAttribute("isleaf");
	
	// create jsNode
	var jsNode;
	if (src != null && src != "")
		jsNode = new WebFXLoadTreeItem(text, src, action, radioValue,isleaf, parent, icon, openIcon,actionDepart);
	else
		jsNode = new WebFXTreeItem(text, action, radioValue,isleaf, parent, icon, openIcon,actionDepart);
...
function _xmlFileLoaded(oXmlDoc, jsParentNode) {
	...
	if( oXmlDoc == null || oXmlDoc.documentElement == null) {
		...
		if(jsParentNode instanceof WebFXLoadTree){
			if(!(webFXTreeConfig.haveInput)){
				if(l == 0){
					var _tagHrefNode = document.getElementById(jsParentNode.id + "-anchor");
					_tagHrefNode.removeAttribute("href");
				}
			}
		}
