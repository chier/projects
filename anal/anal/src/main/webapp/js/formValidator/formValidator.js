...
$.formValidator = 
{
	...
	setOnFocusState : function(tipid,showmsg){
	    var tip = $("#"+tipid);
	    tip.removeClass();
	...
	//设置正确的显示信息
	setSuccessState : function(tipid,showmsg)
	{
	    var tip = $("#"+tipid);
	    tip.removeClass();
	    tip.addClass("onCorrect");
	    tip.html(showmsg);
	...
	pageIsValid : function (validatorgroup)
	{
		...
		{	
			if(elem.settings[0].bind){
				...
				if(returnObj)
				{
					...
					if (!returnObj.isvalid && returnObj.ajax<0) {
						isvalid = false;
						if (thefirstid == ""){
		...
		if(initConfig.validSubmitId!=""){
			isSubmit = $("#"+initConfig.validSubmitId).val();
		}
		if(isvalid||isSubmit=="true")
		{
		...
		{	
			...
			success : function(data){
			    ...
			    else if(res==false)
			    {
			    	...
			    	$.formValidator.setTipState(elem,"onError",res);
			    	setting.onerror = res;
			        setting.isvalid = false;
			    }
			},
...
function getValidatorInstance(validatorgroup){
		if(jQuery_formValidator_initConfig!=null)
		{
		    for(i=0;i<jQuery_formValidator_initConfig.length;i++)
		    {
