...
function onClick(event, treeId, treeNode, clickFlag){
	if(treeNode.type == 1){	// 当选择为定制时
		$(window.parent.document).find("#customRight").attr("src","${base}/manage/custom/customAction!flashTableFrame.${actionExt}?id="+treeNode.id+"&customTitle="+treeNode.name); 
	}
...
function onRemove(e, treeId, treeNode) {
	$.ajax({
			type : "POST",
			async : true,
			url   :  base+"/manage/custom/customAction!removeDirectoryTree."+actionExt,
...
function beforeRename(treeId, treeNode, newName) {
	...
	if (newName.length == 0) {
		alert("节点名称不能为空.");
		var zTree = $.fn.zTree.getZTreeObj("saveLoapTree");
...
function onRename(e, treeId, treeNode) {
	$.ajax({
			type : "POST",
			async : true,
			url   :  base+"/manage/custom/customAction!renameDirectoryTree."+actionExt,
		//	contentType: "application/x-www-form-urlencoded; charset=utf-8", 
...
function addHoverDom(treeId, treeNode) {
	...
	if(treeNode.type == 0 && isAdmin == "true"){
		sObj.after(addStr);
	}
	...
	if (btn) btn.bind("click", function(){
		...
		$.ajax({
			type : "POST",
			async : true,
			url   :  base+"/manage/custom/customAction!createDirectoryTree."+actionExt,
		//	contentType: "application/x-www-form-urlencoded; charset=utf-8", 
...
function beforeCheck(treeId, treeNode) {
	var treeObj = $.fn.zTree.getZTreeObj("saveLoapTree");
...
function viewTreeDirectory(){
	$.ajax({
		type : "POST",
		async : true,
		url   :  base+"/manage/custom/customAction!viewAllTree."+actionExt,
		contentType: "application/x-www-form-urlencoded; charset=utf-8", 
	//	data : "rid="+$("#rid").val(),
		dataType : "json",
		success : function(msg) {
			...
			*/
			...
			$.fn.zTree.init($("#saveLoapTree"), settingSave, _zNodes);
		},
		error : function(XMLHttpRequest, textStatus, errorThrown) {
			// window.top.location=base + "/commons/beforetimeout.jsp";
		}
...
$(document).ready(function(){
	...
	<body style="background-color: #e8edf0;">
		<div>
			 <div class="vip_xinxi">
				<div class="vip_xinxilm"><h1>定制分析</h1></div>
		 	</div> 
			<ul id="saveLoapTree" class="ztree" style="overflow-y:auto;border:none;margin-top:0px;width:94%;height:91%;background-color: #e8edf0;"></ul>
		</div>
	</body>
</html>
