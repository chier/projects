...
if ($("#roleName").val().replace(/[ \t]+/g, "") == "") {
	    ...
	    return false;
	    }else if($("#roleName").val()!=$("#oldRoleName").val()){
...
/**
function checkRoleDec(){

if($("#roleDec").html().replace(/[ \t]+/g, "")==""){
   $("#roleDecNull").css("display","block");
}
} **/
$("document").ready(function() {
            document.getElementById("roleName").select();
            
			var roleType = '${requestScope.roleType}';
			
            if (roleType!=null&&roleType == "0") {
            
			    $("#roleName").attr("disabled", true);
				$("#roleDec").attr("disabled", true);
 ...
 function limtStr(){
   if($("#roleDec").val().length>256){
	...
	<body>
		<span class="ms4">修改角色</span>
		</br>
		<form
			action="${base}/manage/rolemanage/role/roleAction!updateRole.${actionExt}"
			...
			<input type="hidden" id="oldRoleName" name="oldRoleName"
				value="${fn:escapeXml(requestScope.oldRoleName)}" />
			<table width="420" align="center">
				<tr>
					<td width="100" align="right" style="margin-top: 2px">
						角色名称：
						<span style="color: #9d0c0c;">*</span>
					</td>

					<td width="220" align="left" valign="top">
						<input type="text" id="roleName" name="roleName" class="f-text"
							value="${fn:escapeXml(requestScope.roleName)}" maxlength="60"
							size="33" style="width: 220px;" onkeydown="clearError()"
							onmousedown="clearError()" onfocus="clearNameError()" />
					</td>
					<td>
						<!--<span style="color: #9d0c0c;">*</span>-->
					</td>
					<td width="100">
						<font id="roleNameSame" name="roleNameSame"
							style="font-size: 12px; display: none; color: #9d0c0c;">
							<!-- <img
				...
				<tr>
					...
					<td align="left" valign="top">
						...
						<div>
							<font color="#9d0c0c" style="font-size: 12px">角色描述支持256个字符</font>
						</div>
					</td>
					<!-- 
					<td align="left">
				...
				<tr>
					...
					<td align="left" tyle="margin-top: 2px">
						<span class="prompt-msg"> <c:if
								test="${requestScope.errors=='1'}">
		...
		<c:if test="${requestScope.roleType=='0'}">
			<table width="400">
				<tr>
					<td align="center">
		...
		<c:if test="${requestScope.roleType=='1'}">
			<table width="400">
				<tr>
					...
					<td align="left" width="50%">
						<input name="buttonT1" id="buttonT1" type="button" class="bot4"
							value="取消" onclick="closeParent();" />
					</td>
				</tr>
			</table>
		</c:if>
