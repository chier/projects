...
Ext.define('Sencha.controller.BaseController', {
			...
			onIsPage : function(str) {
				...
				return false;
				/*
				 * var isPage = false; var main = Ext.getCmp('main');
				 * main.items.each(function(item) { if(item.id == str) { isPage =
				 * true; } }); return isPage;
				 */
			},
			/**
 ...
 * 返回对象中最大值 ，不保括date属性
 */
function getMaximumByObject(objArr) {
	var max = 0;
 ...
 * 进入到试点基本详情页面
 */
function toBasicInfoIndex() {
	var mainPanel = Ext.getCmp("rootMain");
	var main_2 = Ext.getCmp("BasicInfoRootPanel");
 ...
 * 进入到 上报情况详情页面
 */
function toCaseStatisticsIndex() {
	var mainPanel = Ext.getCmp("rootMain");
	var main_2 = Ext.getCmp("CaseStatisticsRootPanel");
	if (Ext.os.is.Android) {
		mainPanel.setActiveItem(main_2);
	} else {
		mainPanel.animateActiveItem(main_2, {
 ...
 * 进入试点详情页面
 */
function toCaseInfoIndex() {
	console.info("toCaseInfoIndex");
	var mainPanel = Ext.getCmp("rootMain");
	var main_2 = Ext.getCmp("CaseInfoRootPanel");
	if (Ext.os.is.Android) {
		mainPanel.setActiveItem(main_2);
	} else {
		mainPanel.animateActiveItem(main_2, {
 ...
 * 综合分析首页
 */
function toComprehensiveIndex() {
	var mainPanel = Ext.getCmp("rootMain");
	var main_2 = Ext.getCmp("ComprehensiveRootPanel");
	if (Ext.os.is.Android) {
		mainPanel.setActiveItem(main_2);
	} else {
		mainPanel.animateActiveItem(main_2, {
...
function toSurveyIndex() {
	var mainPanel = Ext.getCmp("rootMain");
	var main_2 = Ext.getCmp("SurveyRootPanel");
	if (Ext.os.is.Android) {
		mainPanel.setActiveItem(main_2);
	} else {
		mainPanel.animateActiveItem(main_2, {
 ...
 *            deptCode
 */
function clickComprehensiveIndex(deptCode) {
	...
	Ext.select("div.btn_con3_on", true).each(function() {
		...
		document.getElementById(this.id).setAttribute("class",
				"x-button-normal x-button btn_con3");
	});

	document.getElementById("deptIndex_btn-" + deptCode).setAttribute("class",
			"x-button-normal x-button btn_con3_on");
...
function clickDeptIndexDpet(deptCode) {
	...
	// 框 架存在 bug
	// Ext.get("deptIndex_btn_" +
	// deptCode).removeCls("btn_con3").addCls("btn_con3_on");
...
function usMoney(v) {
	...
	v = (v == Math.floor(v)) ? v + ".00" : ((v * 10 == Math.floor(v * 10)) ? v
			+ "0" : v);
	// v = (v == Math.floor(v)) ? v : ((v * 10 == Math.floor(v * 10)) ? v + "0"
	// : v);
	v = String(v);
	var ps = v.split('.');
	var whole = ps[0];
