...
$().ready(function() {
	...
	var host = document.location.protocol + "//" + document.location.host + contextpath + "/common/dispatch.talent";
...
function getHtml(formId, showData) {
	...
	var msgname = document.getElementById("header_value_msgname").value;
	 $.ajax({
				url :  contextpath + "/common/httpclient/getHtml.talent",
				data : $("#" + formId).serialize(),
				type : "POST",
				dataType : "json",
				headers: {"msgname": msgname},
				cache : false,
				beforeSend : function() {
					tt_beforeSend();
				},
