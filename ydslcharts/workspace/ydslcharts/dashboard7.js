...
MyPanel.prototype.drawImage = function(steps) {
	...
	p.save(function() {		
		...
		img.onload = function(){
		    ...
		    cvs.drawImage(img, 0, 0, 212, 212, 18, 0, 162, 162);			
			p.drawLine();	
	        p.drawSpanner(50+steps);
			

		};
		// img.src = "images/android_biaopan_big200_200.png";
...
MyPanel.prototype.drawLine = function() {
	...
	for (var i = -10; i <= 70; i=i+2) {
		p.save(function() {
			cvs.translate(p.width / 3, p.height/2);
			cvs.rotate(Math.PI / 60 * i);
			p.save(function() {
				cvs.beginPath();
				// 原 是 57  后改成 67
				cvs.lineTo(-(p.height - p.outsideStyle.lineWidth-95) + 10, 0);//设置圆形文本刻度半径大小
				if ((i+10) % 10 === 0) {
					cvs.lineTo(-(p.height - p.outsideStyle.lineWidth-95) + 1, 0);//设置圆形文本刻度半径大小
				}else{
					cvs.lineTo(-(p.height - p.outsideStyle.lineWidth-95) + 3, 0);//设置圆形文本刻度半径大小
				}						
				cvs.strokeStyle = "#FFF";						
				cvs.lineWidth = 1;
				cvs.stroke();				
...
MyPanel.prototype.drawText = function(val) {
	...
	var cvs = this.cvs;
	p.save(function() {
		cvs.strokeText(val, -(p.height - p.outsideStyle.lineWidth-95) + 12, 3);//设置圆形文本半径大小
	});
};
MyPanel.prototype.drawSpanner = function(value) {
	...
	p.save(function() {
		...
		p.save(function() {
			...
			cvs.moveTo(3, -3);
			cvs.lineWidth = 2;
			cvs.lineTo(0, -p.height * 0.35);//设置指针长短 // 原来是0.45 后改成 0.4
			cvs.lineTo(-3, -3);
			cvs.lineTo(3, -3);
			cvs.fillStyle = "red";
			cvs.fill();
...
window.init = function(value,flag,index) {
	...
	document.getElementById("myYibiao").onclick = function(){
		clickCanvasAll();
...
function initMap() {
	...
	myImage.onload = function() {
		...
		// Draw image.	宽是减8   高是减30
		ctx.strokeText('西北地区', 112, 35);
		ctx.strokeText('-', 112, 55);

		ctx.strokeText('西南地区', 112, 105);
		ctx.strokeText('-', 112, 125);

		ctx.strokeText('华北地区', 230, 35);
		ctx.strokeText('-', 230, 55);

		ctx.strokeText('华南地区', 205, 140);
		ctx.strokeText('-', 205, 160);

		ctx.strokeText('华中地区', 320, 110);
		ctx.strokeText('-', 320, 130);
		
		ctx.strokeText('东北地区', 370, 20);
		ctx.strokeText('-', 370, 40);
		
	}

	myImage.src = 'images/map7.png';
