Ext.define('Sencha.controller.DeptInfoController7', {
	...
	onDeptArea : function(scope, number, deptCode, type) {
		...
		var the_param = '{"op":"Dept.area","source_id":"1","view_id":"' + Global.ViewId + '","data":{"year":' + _year + ',"scope":"' + _scope + '","number":' + _number + ',"deptCode":\"' + _deptCode + '\"}}';
					...
					*/
					...
					//ctx.fillStyle     = 'red';
					ctx.fillStyle     = '#fff';
					ctx.fillRect(110, 40, 55, 20);
					
				//	ctx.fillStyle     = '#fff';
					ctx.fillRect(110, 109, 55, 20);
					
				//	ctx.fillStyle     = '#fff';
					ctx.fillRect(228, 38, 55, 20);
					
				//	ctx.fillStyle     = '#fff';
					ctx.fillRect(203, 146, 55, 20);
					
				//	ctx.fillStyle     = '#fff';
					ctx.fillRect(318, 114, 55, 20);
					
					
				//	ctx.fillStyle     = '#fff';
					ctx.fillRect(368, 26, 55, 20);
		...
		Ext.data.JsonP.request({
			...
			callback : function(success, result) {
				if(!result || result.code != 0) {
					...
					ctx.strokeText(usMoney(_xibei), 112, 55);
					ctx.strokeText(usMoney(_xinan), 112, 125);
					ctx.strokeText(usMoney(_huabei), 230, 55);
					ctx.strokeText(usMoney(_huanan), 205, 160);
					ctx.strokeText(usMoney(_huazhong), 320, 130);
					ctx.strokeText(usMoney(_dongbei), 370, 40);
				}
			}
		});
		...
		Ext.data.JsonP.request({
			...
			callback : function(success, result) {
				if(!result || result.code != 0) {
					...
					document.getElementById("emFlightTotal").setAttribute("style", "width:0%");
					document.getElementById("emHotelTotal").setAttribute("style", "width:0%");
